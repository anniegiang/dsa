...
class MinMaxStack {
  ...
  min() {
    return this.minVals[this.minVals.length - 1] || null;
  }

  max() {
    return this.maxVals[this.maxVals.length - 1] || null;
  }
  ...
  pop() {
    ...
    if (this.top.value === this.maxVals[this.maxVals.length - 1].value) {
      this.maxVals.pop();
    }

    if (this.top.value === this.minVals[this.minVals.length - 1].value) {
      this.minVals.pop();
    }
