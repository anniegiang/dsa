...
function stepper(nums) {
  ...
  for (let start = 0; start < table.length; start++) {
    ...
    for (let step = start; step <= nums[start]; step++) {
      table[step] = true;
    }
  }
...
function maxNonAdjacentSum(nums) {
  ...
  for (let i = 1; i < table.length; i++) {
    const twoAwayFromLeft = table[i - 2] === undefined ? 0 : table[i - 2];
    const currentSum = twoAwayFromLeft + nums[i];
    const oneAwayFromLeft = table[i - 1];
    table[i] = Math.max(currentSum, oneAwayFromLeft);
  }
